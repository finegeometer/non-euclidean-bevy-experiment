...
layout(location = 1) in vec4 v_WorldNormal;
...
// distanceAttenuation is simply the falloff of light intensity
// combined with a smooth attenuation at the edge of the light radius
// 
// light radius is a non-physical construct for efficiency purposes,
// because otherwise every light affects every fragment in the scene
float getDistanceAttenuation(const float dotPosLight, float inverseRadiusSquared) {
    // The reciprocal falloff is proportional to the surface area
    // of a sphere of photons, centered at the light source,
    // that passes through your location.
    // 
    //     L....
    //     │    ..   
    //     │      ..
    ...
    // r² + (L·P)² = 1
    //
    float recip_falloff = 1.0 - dotPosLight * dotPosLight;
    float factor = recip_falloff * inverseRadiusSquared;
    float smoothFactor = saturate(1.0 - factor * factor);
    float attenuation = smoothFactor * smoothFactor;
    return attenuation * 1.0 / max(recip_falloff, 1e-4);
}
...
float D_GGX(float roughness, float NoH, const vec4 h) {
...
vec3 specular(vec3 f0, float roughness, const vec4 h, float NoV, float NoL,
              float NoH, float LoH) {
...
void main() {
    vec4 WorldPosition = normalize(v_WorldPosition);
...
#    endif
    ...
    vec4 V = CameraPos - WorldPosition;
    V -= WorldPosition * dot(V, WorldPosition);
    V = normalize(V);
    ...
    for (int i = 0; i < int(NumLights.x) && i < MAX_LIGHTS; ++i) {
        ...
        vec4 L = normalize(light.pos) - WorldPosition;
        L -= WorldPosition * dot(L, WorldPosition);
        L = normalize(L);

        float rangeAttenuation =
            getDistanceAttenuation(dot(WorldPosition, normalize(light.pos)), dot(light.pos, light.pos));
